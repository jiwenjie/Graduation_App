...
/**
 * 有清除功能的 EditText：只有当有焦点和文本不为空的时候才显示
 */
public class ClearEditText extends android.support.v7.widget.AppCompatEditText
    ...
    private void initCustomAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //是否展示底部线
        showBottomLine = ta.getBoolean(R.styleable.ClearEditText_clear_showBottomLine, true);
        //底部线的颜色
        defaultBottomLineColor = ta.getColor(R.styleable.ClearEditText_clear_bottomLineColor, Color.parseColor("#E6E6E6"));
        //底部线的宽度dp
        bottomLineWidth = ta.getDimension(R.styleable.ClearEditText_clear_bottomLine_width, dip2px(context, 1));
        //左边按钮的大小dp
        leftDrawableSize = ta.getDimension(R.styleable.ClearEditText_clear_leftDrawableSize, dip2px(context, 20));
        //清除功能是否可用
        disableClean = ta.getBoolean(R.styleable.ClearEditText_clear_disableClear, true);
        //右边按钮的大小dp
        rightDrawableSize = ta.getDimension(R.styleable.ClearEditText_clear_rightDrawableSize, dip2px(context, 20));
        ta.recycle();
    }
